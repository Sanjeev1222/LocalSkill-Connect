...
const updateBookingStatus = asyncHandler(async (req, res) => {
  ...
  if (status === 'completed') {
    ...
    const otpResult = verifyOTP(booking._id.toString(), otp, 'booking_complete');
    if (!otpResult.valid) {
...
const sendBookingCompleteOTP = asyncHandler(async (req, res) => {
  ...
  const otp = storeOTP(booking._id.toString(), 'booking_complete');

  // In production send SMS to booking.user.phone. For demo return it.
  res.json({
    success: true,
    message: `OTP sent to user ${booking.user?.name}'s phone`,
    demoOTP: otp,
    userPhone: booking.user?.phone ? `****${booking.user.phone.slice(-4)}` : 'Not available'
  });
});
