...
const { asyncHandler, storeOTP, verifyOTP } = require('../utils/helpers');
...
const updateRentalStatus = asyncHandler(async (req, res) => {
  ...
  if (status === 'returned') {
    ...
    const otpResult = verifyOTP(rental._id.toString(), otp, 'rental_return');
    if (!otpResult.valid) {
...
// ─── Send OTP for tool return (sent to tool owner's phone) ───
const sendRentalReturnOTP = asyncHandler(async (req, res) => {
  ...
  const otp = storeOTP(rental._id.toString(), 'rental_return');

  // In production send SMS to tool owner's phone. For demo return it.
  res.json({
    success: true,
    message: `OTP sent to tool owner ${rental.toolOwner?.user?.name}'s phone`,
    demoOTP: otp,
    ownerPhone: rental.toolOwner?.user?.phone ? `****${rental.toolOwner.user.phone.slice(-4)}` : 'Not available'
  });
});
