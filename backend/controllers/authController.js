...
const { generateToken, asyncHandler, validatePhone, storeOTP, verifyOTP } = require('../utils/helpers');
...
const sendRegisterOTP = asyncHandler(async (req, res) => {
  ...
  const otp = storeOTP(phone, 'register');

  // In production, send OTP via SMS service. For demo, return it in response.
  res.json({
    success: true,
    message: 'OTP sent successfully',
    demoOTP: otp // Demo only — remove in production
  });
});
...
const verifyRegisterOTP = asyncHandler(async (req, res) => {
  ...
  const result = verifyOTP(phone, otp, 'register');
  if (!result.valid) {
